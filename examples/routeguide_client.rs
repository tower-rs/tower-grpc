...
fn main() {
    ...
    let done = h2.new_service()
        ...
        .and_then(move |orig_service| {
            ...
            let grpc = tower_grpc::client::Client::new(StupidCodec::<Point, Feature>::new(), service);
            let get_feature = GetFeature::new(grpc);


            let service = AddOrigin(orig_service);
            let grpc = tower_grpc::client::Client::new(StupidCodec::<Rectangle, Feature>::new(), service);
